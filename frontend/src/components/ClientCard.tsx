...
interface ClientCardProps {
  ...
  onOrderTypeChange: (value: "delivery" | "retirada") => void;
  notes: string;
...
export function ClientCard({
  ...
  onCheckout,
  isSending,
}: ClientCardProps) { // Aplicamos a interface aqui
  
  // Agora o TS sabe que 'item' possui price e quantity como números
  ...
  return (
    <div>
      ...
      <div className="mt-4 space-y-4">
        ...
        <select
          value={orderType}
          onChange={(event) => onOrderTypeChange(event.target.value as "delivery" | "retirada")}
          className="w-full rounded-2xl border border-slate-200 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-orange-400"
        >
